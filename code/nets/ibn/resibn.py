    ...
    def _make_layer(self, block, planes, blocks, strip_kernel_size, stride=1, ibn=None, use_global=True,
        ...
        for i in range(1, blocks):
            ...
            if use_global:
                # print(self.inplanes)
                layers.append(
                    AsymGlobalAttn(self.inplanes, strip_kernel_size)  # planes = 64
                )
...
def resnet50_ibn_b(pretrained=False, **kwargs):
    ...
    model = ResNet_IBN(block=Bottleneck_IBN,
                       layers=[3, 4, 6, 3],
                       ibn_cfg=('b', 'b', None, None),
                       use_global=(True, True, False, False),
                       **kwargs)
    if pretrained:
        model_dict = model.state_dict()
...
if __name__ == '__main__':
    model = resnet18_ibn_b(pretrained=True)
    x1 = torch.randn(4, 4, 1024, 1024)
    output = model(x1)
    print(len(output))
